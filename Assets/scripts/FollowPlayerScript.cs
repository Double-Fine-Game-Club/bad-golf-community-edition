...
public class FollowPlayerScript : MonoBehaviour {
	...
	public float camCollideRadius = 1.5f;
	...
	void LateUpdate() {
		...
		if (camLean) {
			...
			colliding = Physics.CheckSphere(transform.position, camCollideRadius);
			
			if(colliding) {
				// move toward zero distance on x-y from target
				newDistance = Mathf.Lerp(currentDistance, 0, leanDamping * Time.deltaTime);
				colliding = true;
			}
			else {
				// move toward user defined distance on x-y from target
				newDistance = Mathf.Lerp(currentDistance, distance, leanDamping * Time.deltaTime);
			}
			
			// calculate the required height to retain the retain the same camera distance [b = sqrt(h^2 - a^2)]
			wantedHeight = Mathf.Sqrt(wantedCamDistanceSquared - newDistance * newDistance) + cameraTilt;
		}
		...
		// reset camera position if it would go from a non-colliding state to a colliding state
		if (camLean) {
			if(!colliding && Physics.CheckSphere(transform.position, camCollideRadius)) {
